...
class human_expert_env(Env):
    ...
    def step(self):
        ...
        reward = self.reward(new_plane_state['health_level'], new_enemy_state['health_level'], new_plane_state['target_locked'], new_enemy_state['target_locked'])
        terminate = True if self.terminate(new_plane_state['health_level'], new_enemy_state['health_level']) else False
    ...
    def reward(self, health, enemy_health, lock, enemy_lock):
        if enemy_health <= 0.9:
            reward = 50
        elif health <= 0.8:
            reward = -50
