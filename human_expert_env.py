...
class human_expert_env(Env):
    ...
    def step(self):
        ...
        action = [action_get["ROLL"], action_get["PITCH"], action_get["YAW"], action_get["THRUST"], action_get["FIRE"]]
        ...
        # ______________________end of obs__________________________ #
        if self.last_obs is None:
            self.last_obs = new_ob
        self.replay_buffer.add(self.last_obs, new_ob, action, reward, terminate, [{}])
        self.last_obs = new_ob

        if self.replay_buffer.full:
            now = datetime.now()
            now = now.strftime("%Y_%m_%d_%H_%M_%S")
            path = "D:\desktop\code\save_buffer" + "\\" + now + ".pkl"
            save_to_pkl(path, self.replay_buffer)
            self.replay_buffer = ReplayBuffer(self.buffer_size, self.observation_space, self.action_space)
            self.finish_collecting = True
            terminate = 1
    ...
    def reset(self):
        ...
        self.missle_count = 0
        self.step_game = 0
        range_enemy = hg.Vec3(500, 500, 500)
        center_enemy = hg.Vec3(1000, 2000, 3000)
        ...
        center_plane = hg.Vec3(1000, 2000, 1500)
        y_orientations_range_plane = hg.Vec2(-circle_range, circle_range)
        ...
        current_loc = [uni_enemy_x, uni_enemy_y, uni_enemy_z, uni_enemy_rad, uni_plane_x, uni_plane_y, uni_plane_z,
                       uni_plane_rad]
        ...
        if self.finish_collecting:
            self.reset_start_point.pop(-1)
            now = datetime.now()
            now = now.strftime("%Y_%m_%d_%H_%M_%S")
            path = "D:\desktop\code\save_buffer" + "\\" + now + '_reset_locations' + ".pkl"
            save_to_pkl(path, self.reset_start_point)
            self.reset_start_point = []
            self.finish_collecting = False
        print(uni_plane_rad)
        return new_ob
