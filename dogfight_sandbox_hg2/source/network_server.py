...
def init_server(main_):
	...
	commands_functions = {
		...
		"GET_FINISH_FLAG": get_finish_flag,
		"GET_GAMEPAD_ACTION": get_gamepad_action, #by hs
		...
		"RESET_DOGFIGHT_LOC": reset_dogfight_loc,
...
def reset_dogfight_loc(args):
	machine = main.destroyables_items[args["plane_id"]]
	print(machine.get_thrust_level())
	# # loc = args['loc']
	# # center = hg.Vec3(loc['center'][0], loc['center'][1], loc['center'][2])
	# center = hg.Vec3(0,1000,0)
	# # center_range = hg.Vec3(loc['range'][0], loc['range'][1], loc['range'][2])
	# center_range = hg.Vec3(500, 500, 500)
	# # y_orientations_range = hg.Vec3(loc['y_orientations_range'][0], loc['y_orientations_range'][1])
	# y_orientations_range = hg.Vec3(0, 0)
	# speed_range = hg.Vec3(300, 300)
	# #
	# machine.reset_matrix(hg.Vec3(uniform(center.x - center_range.x / 2, center.x + center_range.x / 2),
	# 							 uniform(center.y - center_range.y / 2, center.y + center_range.y / 2),
	# 							 uniform(center.z - center_range.z / 2, center.z + center_range.z / 2)),
	# 					 hg.Vec3(0, radians(uniform(y_orientations_range.x, y_orientations_range.y)), 0))
	# machine.set_linear_speed(uniform(speed_range.x, speed_range.y))
	print('relocate111')

def get_finish_flag(args):
	flag = main.wait_till_finish
	socket_lib.send_message(str.encode(json.dumps(flag)))
...
def get_plane_state(args):
	...
	state = {
		...
		"pitch_attitude": machine.pitch_attitude,
		# "roll_attitude": machine.get_roll_attitude(),
		"roll_attitude": machine.roll_attitude,
...
def get_gamepad_action(args):
	machine = main.destroyables_items[args["plane_id"]]
	reset_missile = args["reset_missile_count"]
	auc = machine.get_device("UserControlDevice")
	if reset_missile:
		auc.current_fire_missile_count = 0
	action = {
		"ROLL": auc.current_roll,
		"PITCH": auc.current_pitch,
		"YAW": auc.current_yaw,
		"THRUST": auc.current_thrust,
		"FIRE": auc.current_fire_missile_count,
	}
	socket_lib.send_message(str.encode(json.dumps(action)))

def send_gamepad_action(args):
	machine = main.destroyables_items[args["plane_id"]]
	reset_missile = args["reset_missile_count"]
	auc = machine.get_device("UserControlDevice")
	if reset_missile:
