...
class TargettingDevice(MachineDevice):
    ...
    def __init__(self, name, machine, start_state=True):
        ...
        self.target_lock_delay = hg.Vec2(1, 1)  # Target lock delay in lock range
    ...
    def get_target_name(self):
        if self.target_id <= 0 or len(self.targets) == 0:
            # print('targetid is ', self.target_id, 'len of target is', len(self.targets))
            return ""
        else:
    ...
    def update_target_lock(self, dts):
        if self.target_id > 0:
            ...
            if self.target_angle < front_lock_angle and self.target_lock_range.x < self.target_distance < self.target_lock_range.y:
                t = (self.target_distance - self.target_lock_range.x) / (self.target_lock_range.y - self.target_lock_range.x)
...
class AircraftUserControlDevice(ControlDevice):
    ...
    def __init__(self, name, machine, inputs_mapping_file, control_mode=ControlDevice.CM_KEYBOARD, start_state=False):
        ...
        self.set_control_mode(control_mode)
        self.current_roll = 0      #by hs
        self.current_pitch = 0     #by hs
        self.current_yaw = 0       #by hs
        self.current_thrust = 0    #by hs
        self.current_fire_missile_count = 0 #by hs
        self.update_frequency = 5  #by hs
        self.collect_expert_sample = True
    ...
    def update_cm_gamepad(self, dts):
        self.current_fire_missile_count = 0
    ...
    def set_thrust_level_gp(self, value):
        ...
        if v < - epsilon or v > epsilon:
            ...
            self.current_thrust = self.machine.thrust_level_dest + v * 0.01
    ...
    def fire_missile_gp(self, value):
        if ControlDevice.gamepad.Pressed(value):
            ...
            if md is not None:
                md.fire_missile()
                self.current_fire_missile_count += 1
...
class AircraftIAControlDevice(ControlDevice):
    ...
    def update_IA_liftoff(self, aircraft, dts):
        ...
        if self.IA_liftoff_delay <= 0:  #结束了起飞模式以后正式进入空战的状态
            if aircraft.thrust_level < 1 or not aircraft.post_combustion:  #如果油门没1或者没开加力
                aircraft.set_brake_level(0)
                aircraft.set_flaps_level(1)
                aircraft.set_thrust_level(1)
                aircraft.activate_post_combustion()
        ...
        if autopilot is not None:
            ...
            if speed < aircraft.minimum_flight_speed:  #失速保护
                ...
                if td.target_id > 0:
                    ...
                    if td.target_locked:  #在锁定目标之后进行一些延时，再发射导弹
                        if md is not None:
                            if self.IA_fire_missile_cptr <= 0:
                                md.fire_missile()
                                self.IA_fire_missile_cptr = self.IA_fire_missiles_delay  # 发射导弹之间有10的CD
                            else:
    ...
    def update(self, dts):
        if self.activated:  #这里只是做出一些切换模式的选择
            if self.flag_user_control and self.machine.has_focus():
                if self.control_mode == ControlDevice.CM_KEYBOARD:
                    self.update_cm_keyboard(dts)
                elif self.control_mode == ControlDevice.CM_GAMEPAD:
